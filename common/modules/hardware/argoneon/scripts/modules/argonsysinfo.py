...
def argonsysinfo_gethddtemp():
	...
	hddtempcmd = " /run/current-system/sw/bin/smartctl"
	if os.path.exists(hddtempcmd) == False:
		# Fallback for now
		hddtempcmd = " /run/current-system/sw/bin/hddtemp"
...
def argonsysinfo_getdevhddtemp(hddtempcmd, curdev):
	cmdstr = ""
	if hddtempcmd == " /run/current-system/sw/bin/hddtemp":
		cmdstr = " /run/current-system/sw/bin/hddtemp -n sata:/dev/"+curdev
	elif hddtempcmd == " /run/current-system/sw/bin/smartctl":
		cmdstr = " /run/current-system/sw/bin/smartctl -d sat -A /dev/"+curdev+" | grep Temperature_Celsius | awk '{print $10}'"
