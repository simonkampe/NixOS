...
FAN_CONFIGFILE = "/etc/argon-fan.conf"
FAN_HDD_CONFIGFILE = "/etc/argon-fan-hdd.conf"
...
def temp_check():
	...
	tmpconfig = load_config(FAN_CONFIGFILE)
	if len(tmpconfig) > 0:
		fanconfig = tmpconfig

	if os.path.isfile(FAN_HDD_CONFIGFILE):
		tmpconfig = load_config(FAN_HDD_CONFIGFILE)
		if len(tmpconfig) > 0:
...
if len(sys.argv) > 1:
	...
	if cmd == "SHUTDOWN":
		...
		try:
			ipcq = Queue()
			fancontrol = Thread(target = temp_check)
			fancontrol.start()
			powerbutton = Thread(target = argonpowerbutton_monitor, args =(ipcq, ))
			powerbutton.start()
			if OLED_ENABLED == True:
				oled = Thread(target = display_loop, args =(ipcq, ))
				oled.start()
			ipcq.join()
		except Exception:
			sys.exit(1)